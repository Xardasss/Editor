            ...
            if (this.parentNode.querySelector('.containerTags') && (!this.parentNode.querySelector('.textNode .backlight'))) {
                ...
                if (!this.parentNode.querySelector('.textNode span')) {
                    ...
                    for (var i = 0; i < contTag.children.length; i++) {
                        ...
                        reg = new RegExp('\\' +mas[i], "ig");

                        if (i == 0) {
                            conttext = conttext.replace(reg, "&nbsp<span class='backlight' contenteditable='false' spellcheck='false'>" + mas[i] + "</span>&nbsp")
                        } else if (i != 0 && i != contTag.children.length - 1)
                            conttext = conttext.replace(reg, "<span class='backlight' contenteditable='false' spellcheck='false'>" + mas[i] + "</span>&nbsp")

                        else if (i == contTag.children.length - 1) {
                            conttext = conttext.replace(reg, "<span class='backlight' contenteditable='false' spellcheck='false'>" + mas[i] + "</span>&nbsp")
